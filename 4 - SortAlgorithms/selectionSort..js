/* Select Sort explained 
Think of select sort as taking one element from the unsorted list (the selection). Add this to a type of "stack". This reduces the length of the unsorted list by one.
Compare the selected item to all items in the unsorted list. If the selected item is > greater than item in the unsorted list, swap them and continue the comparision
until reaching the end of the unsorted list. Then select the next item at the begining of the unsorted list, reducing the length of the unsorted list and begin the comparison again
If you run out of items in the unsorted list all the items selected are in order in the "stack".
...
Notice also, that the JavaScript array.sort() method appears to do a type conversion on numbers so sorting has 1, 10, 2, 20 as ordered.
...
function selectSort(unorderedNo) {
    ...
    // The built-in JavaScript sort has a few features that would enable this sort in fewer lines, but the you would
    // defeat the purpose of learning the selecionSort algorithm.
